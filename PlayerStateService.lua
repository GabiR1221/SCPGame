...
function Service.IsAlive(self:any,player:Player):boolean local r:Record?=self.Records[player]; if r==nil then return false end; return r.State~="Dead" and r.Humanoid.Health>0 end
function Service.CanBegin(self:any,player:Player):boolean local r:Record?=self.Records[player]; if r==nil then return false end; return r.State=="Normal" and not self:IsLocked(player,"Interaction") end
function Service.CanUseTools(self:any,player:Player):boolean local r:Record?=self.Records[player]; if r==nil or r.State~="Normal" or r.Humanoid.Health<=0 then return false end; if self:IsLocked(player,"Tool") or self:IsLocked(player,"Inventory") then return false end; return true end
