...
function S.new(states:any,tracker:any,remote:RemoteEvent):any return setmetatable({States=states,Tracker=tracker,Remote=remote,Lockers={} :: {[Instance]:number},Occupants={} :: {[Instance]:Player},Records={} :: {[Player]:Record},Rates={} :: {[Player]:number},Generation={} :: {[Player]:number}},S) end
...
function S._cleanup(self:any,player:Player,reason:string)
	...
	if record.Root.Parent then
		record.Root.AssemblyLinearVelocity=Vector3.zero; record.Root.AssemblyAngularVelocity=Vector3.zero; record.Root.Anchored=record.WasAnchored
		if not record.Root.Anchored then record.Root:SetNetworkOwnershipAuto() end
	end
...
function S.Enter(self:any,player:Player,locker:Instance)
	...
	local character=player.Character; if not character then return end; local rootValue=character:FindFirstChild("HumanoidRootPart"); local humanoid=character:FindFirstChildOfClass("Humanoid"); local hidden=attachment(locker,"HiddenPoint"); local exitPoint=attachment(locker,"ExitPoint"); if not rootValue or not rootValue:IsA("BasePart") or not humanoid or not hidden or not exitPoint then warn(`[HidingService] {locker:GetFullName()} requires HiddenPoint and ExitPoint Attachments`); return end
	...
	humanoid:UnequipTools(); local wasAnchored=rootValue.Anchored; rootValue.AssemblyLinearVelocity=Vector3.zero; rootValue.AssemblyAngularVelocity=Vector3.zero; rootValue.CFrame=hidden.WorldCFrame; rootValue.Anchored=true
	self.Records[player]={Locker=locker,RoomIndex=roomIndex,Root=rootValue,WasAnchored=wasAnchored,Token=token,Generation=generation,Phase="Entering"}
	task.delay(Config.EnterDelay,function() local record:Record?=self.Records[player]; if not record or record.Generation~=generation or not locker.Parent or humanoid.Health<=0 then self:_cleanup(player,"EnterCancelled"); return end; record.Phase="Hidden"; local camera=attachment(locker,"CameraPoint"); self.Remote:FireClient(player,{Kind="Hidden",CameraCFrame=if camera then camera.WorldCFrame else hidden.WorldCFrame,Fov=Config.CameraFov}) end)
end
function S.Exit(self:any,player:Player)
	local now=workspace:GetServerTimeNow(); if now<(self.Rates[player] or 0) then return end; self.Rates[player]=now+Config.RequestCooldown; local record:Record?=self.Records[player]; if not record or record.Phase~="Hidden" then return end; record.Phase="Exiting"; local generation=record.Generation
	self.Remote:FireClient(player,{Kind="Exiting"}); task.delay(Config.ExitDelay,function() local current:Record?=self.Records[player]; if not current or current.Generation~=generation then return end; local exitPoint=attachment(current.Locker,"ExitPoint"); if exitPoint and current.Root.Parent then current.Root.CFrame=exitPoint.WorldCFrame; current.Root.AssemblyLinearVelocity=Vector3.zero; current.Root.AssemblyAngularVelocity=Vector3.zero end; current.Locker:SetAttribute("InteractionText","Hide"); self:_cleanup(player,"Exited") end)
end
